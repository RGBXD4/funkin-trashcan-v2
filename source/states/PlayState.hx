...
class PlayState extends MusicBeatState
{
	...
	function openPauseMenu()
	{
		...
		if (chartingMode) 	openSubState(new ChartingPauseSubState());
	...
	function doDeathCheck(?skipHealthCheck:Bool = false) {
		if (((skipHealthCheck && instakillOnMiss) || health <= 0) && !practiceMode && !isDead && gameOverTimer == null)
		{
			...
			if(ret != LuaUtils.Function_Stop)
			{
				...
				if(autoUpdateRPC) 
					if (storyDifficultyText == "Normal") DiscordClient.changePresence("Game Over - " + detailsText, discordScore, iconP2.getCharacter(), true, discLogo.toLowerCase());
					else DiscordClient.changePresence("Game Over - " + detailsText, discordScore + " (" + storyDifficultyText + ")", iconP2.getCharacter(), true, discLogo.toLowerCase());
				#end
				isDead = true;
				return true;
			}
